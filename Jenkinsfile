...
def dockerRuntime(command) {
    def dockerImage = docker.image('hub.ihr.local/java:1.8-dev')
    dockerArgs = '-u root ' + '--network=host ' +
            '-v /data/jenkins_home/.cache/.gradle:/root/.gradle ' +
...
node {
    try {
        ...
        stage('Unit Test') {
            // 通过docker插件运行项目测试
            dockerRuntime('./gradlew clean test')
        }

        stage('Code Analysis') {
           dockerRuntime('./gradlew jacocoTestCoverageVerification sonar')
        }

        stage('Code Build') {
            // 通过docker插件运行项目构建
            dockerRuntime('./gradlew clean build -x test')
        }
